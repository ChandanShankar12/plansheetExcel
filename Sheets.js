...
const Sheets = {
    ...
    init(rootPath) {
        ...
        this.convertedPath = path.join(__dirname, 'ProcessFiles/test/Post-Processed/');
        return this;
    },

    extensionConversion(rootPath) {
        try {
...
const Column = {
    ...
    async getColumnData(filePath, columnKey, sheetName = null, cellRange = null) {
        try {
            ...
            const availableSheets = this.workbook.worksheets.map(sheet => sheet.name);
            console.log('Available sheets:', availableSheets);

            // Get the worksheet (active sheet if sheetName not provided)
            let worksheet;
            if (sheetName) {
                worksheet = this.workbook.getWorksheet(sheetName);
                if (!worksheet) {
            ...
            const getColumnData = (columnKey, cellRange) => {
                ...
                column.eachCell((cell, rowNumber) => {
                    ...
                    columnData[cellAddress] = {
                        value: cell.value,
                        type: cell.type
                    };
                });

                // Create Data directory in ProcessFiles if it doesn't exist
                const outputDir = path.join(__dirname, '/ProcessFiles/Data');
                if (!fs.existsSync(outputDir)) {
                    fs.mkdirSync(outputDir, { recursive: true });
                }

                // Save complete column data to JSON
                let columnDataOutput = {
                ...
                if (cellRange) {
                    ...
                    Object.entries(columnData).forEach(([cellAddress, cellInfo]) => {
                        const rowNumber = parseInt(cellAddress.match(/\d+/)[0]);
                        if (rowNumber >= startRow && rowNumber <= endRow) {
                            rangeColumnCellData[cellAddress] = cellInfo;
                        }
                    });

                    columnDataOutput.rangeData = rangeColumnCellData;

                    // Save merged data to a new JSON file
                    const mergedDataPath = path.join(outputDir, `merged_data_${columnKey}.json`);
                    fs.writeFileSync(mergedDataPath, JSON.stringify(columnDataOutput, null, 2));
                    console.log(`Merged data saved to: ${mergedDataPath}`);

                    return columnDataOutput.rangeData;
                }

                return columnDataOutput;
            };

            const result = getColumnData(columnKey, cellRange);
            console.log('Complete Column Data:', result);
